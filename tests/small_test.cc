...
// 辅助函数：翻转32位整数的位
uint32_t reverse_word(uint32_t w) {
...
// 标准CRC32实现（带输入输出翻转）
// 使用多项式0x04C11DB7，符合主流规范
uint32_t crc32_with_reversal(const uint8_t* data, size_t length) {
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        // 与CRC的低8位异或后查表
        crc = (crc >> 8) ^ crc_table[(crc & 0xFF) ^ reversed_byte];
    }

    // 输出翻转：先与0xFFFFFFFF异或，再翻转整个32位结果
    return reverse_word(crc ^ 0xFFFFFFFF);
}

// CRC32实现（不带输入输出翻转）
// 使用相同多项式0x04C11DB7，但不进行位翻转
uint32_t crc32_without_reversal(const uint8_t* data, size_t length) {
    uint32_t crc = 0x00000000; // 初始值与带翻转版本不同
    ...
    static const uint32_t crc_table[256] = {
        ...
        0xAE3AFBA2, 0xAAFB0615, 0xA7B820CC, 0xA3793D7B, 0x9B3660C6, 0x9FF77D71,
        ...
        0xEA23F0AF, 0xEEE2ED18, 0xF0A5BD1D, 0xF464A0AA, 0xF9278673, 0xFDE69BCD,
    ...
    for (size_t i = 0; i < length; ++i) {
        // 不进行输入翻转，直接使用原始字节
        // 注意这里的移位方向与带翻转版本相反（左移而非右移）
        crc = (crc << 8) ^ crc_table[(crc >> 24) ^ data[i]];
    }

    // 不进行输出翻转，仅与0xFFFFFFFF异或
...
int main(int argc,char** argv){
    ...
    std::cout<<"test_data size:" << test_data.size() << std::endl;
    std::cout<<"test_data length:" << test_data.length() << std::endl;
    uint32_t crc32_ieee = zpkg::crc32_ieee(reinterpret_cast<const uint8_t*>(test_data.data()),test_data.size());
    uint32_t crc32_mpeg = zpkg::crc32_mpeg2(reinterpret_cast<const uint8_t*>(test_data.data()),test_data.size());

    std::cout<< "crc32_ieee:" << crc32_ieee << std::endl;
    std::cout<< "crc32_mpeg:" << crc32_mpeg << std::endl;

    uint32_t crc32_reverse = crc32_with_reversal(reinterpret_cast<const uint8_t*>(test_data.data()),test_data.size());
    uint32_t crc32_no_reverse = crc32_without_reversal(reinterpret_cast<const uint8_t*>(test_data.data()),test_data.size());

    std::cout<< "crc32_reverse:" << crc32_reverse << std::endl;
    std::cout<< "crc32_no_reverse:" << crc32_no_reverse << std::endl;

    CRC32 crc32;
    std::cout << "crc32 hash:" << crc32(test_data) << std::endl;
    uint8_t buffer[4];
    crc32.getHash(buffer);
    std::cout << "crc32 be:" << Z_RBE32(buffer) << std::endl;
    std::cout << "crc32 le:" << Z_RLE32(buffer) << std::endl;
    return 0;
